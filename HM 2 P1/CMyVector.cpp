...
CMyVektor CMyVektor::gradientenVerfahren(CMyVektor x, double (*function)(CMyVektor x), double lambda) {
	...
	for (int i = 0; i <= 25; i++) {
		...
		std::cout << "Schritt " << i << ":" << std::endl << std::endl <<
			...
			"f(x) = " << func << std::endl <<
			"grad f(x) = " << grad.print() << std::endl <<
			"||grad f(x)|| = " << len << std::endl << std::endl <<
			"x_neu = " << x_neu.print() << std::endl <<
			"f(x_neu) = " << func_neu << std::endl;
		std::cout << std::endl;
		if (func_neu > func) {
			CMyVektor xtest;
			...
			std::cout << "Test mit doppelter Schrittweite (lambda = " << lambdatestdouble << "):" << std::endl <<
				"x_test = " << xtest.print() << std::endl <<
				"f(x_test) = " << func_neu_test << std::endl << std::endl;
			...
			while (func > func_neu) {
				...
				func_neu = function(x_neu);
				std::cout << "x_neu: " << x_neu.print() << std::endl <<
					"f(x_neu): = " << func_neu << std::endl;
			}
